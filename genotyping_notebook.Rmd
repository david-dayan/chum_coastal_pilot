...
#LOCAL R
# Some samples were actually run in triplicate which completely broke the script below
...
#find the triplicate samples, and remove just one of each (lowest on target)
count(genos_0.11, sample_simple) %>%
  arrange(desc(n))
 
genos_0.11 %<>% 
  filter(Sample != "OkeCC19NEHR_0001_CAAACC_TTTGTC_S192_R1_001") %>%
  filter(Sample != "OkeCC19TILR_0019_CTGATA_TCCTGC_S255_R1_001")
...
10 monomorphic markers removed
...
134 individuals X 320 Markers  
Depths: Mean-1422, median-1050, sd-1321  
% On Target: 25.4% (i.e 25.4% of the total number of reads from individuals in the final dataset contribute to called genotypes) 
...
genos_2.1 <- as.matrix(genos_2.1[,c(7:326)]) #caution hardcoding, make sure you select just your marker columns
...
genos_2.0 %<>%
  ...
  select(-c(Sample, adapter, on_target_final))
